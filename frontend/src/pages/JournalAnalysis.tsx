...
import { AlertTriangle, BookOpen, RefreshCcw, Search, ArrowLeft } from 'lucide-react'
...
import type { AnalysisResponse, SearchResult } from '../types/api'
...
export function JournalAnalysis() {
  const [query, setQuery] = useState('deep learning journal papers')
  ...
  const rankedItems = analysisResponse.rankedHeadings
  ...
  async function handleAnalyze() {
    ...
    try {
      const [analysis, serpResults] = await Promise.all([
        analyzeQuery({ query, limit, analysisType: 'JOURNAL' }),
        ...
        <section className="hero-panel">
          <div className="panel panel--primary">
            <span className="eyebrow">
              <BookOpen size={14} />
              Module 2 — Deep learning journals
            </span>
            <h1 className="title">Journal Heading Analyzer</h1>
            <p className="subtitle">
              Search for deep learning journal papers, crawl them concurrently, and extract common
              sub-headings like abstract, methodology, results, and conclusion. Results are ranked by
              frequency.
            </p>

            <div className="hero-actions">
              <button className="button button--primary" type="button" onClick={handleAnalyze} disabled={loading}>
                {loading ? 'Analyzing documents...' : 'Analyze Journal Papers'}
              </button>
            </div>
          </div>
        ...
        <div className="grid" style={{ marginTop: 20 }}>
          <section className="panel section">
            <div className="section__header">
              <h2 className="section__title">Search Journal Papers</h2>
              <span className="section__caption">Analysis type: JOURNAL</span>
            </div>
            <div className="form form--journal">
              <input
                className="field"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Deep learning models, neural network architectures..."
          ...
          <section className="cards">
            ...
            <StatCard label="Headings Found" value={totalRanked} subtext="Ranked journal headings" />
            <StatCard label="Top Share" value={`${topPercentage.toFixed(2)}%`} subtext="Most frequent heading" />
          </section>

          <SummaryPanel summary={analysisResponse.summary} />

          <div className="grid grid--two">
            <RankingBarChart title="Journal Heading Ranking" data={chartData} />
            <RankingPieChart
              title="Journal Heading Distribution"
