...
import { AlertTriangle, FileSearch, RefreshCcw, Search, ArrowLeft } from 'lucide-react'
...
export function CrimeAnalysis() {
  ...
  const rankedItems = analysisResponse.rankedFeatures
  ...
  async function handleAnalyze() {
    ...
    try {
      const [analysis, serpResults] = await Promise.all([
        analyzeQuery({ query, limit, analysisType: 'CRIME' }),
        ...
        <section className="hero-panel">
          <div className="panel panel--primary">
            <span className="eyebrow">
              <FileSearch size={14} />
              Module 1 — Crime reporting
            </span>
            <h1 className="title">Crime-Reporting Feature Analyzer</h1>
            <p className="subtitle">
              Search for crime-reporting papers, crawl them concurrently, and extract distinctive features
              like headlines, locations, suspects, and court proceedings. Results are ranked by frequency.
            </p>

            <div className="hero-actions">
              <button className="button button--primary" type="button" onClick={handleAnalyze} disabled={loading}>
                {loading ? 'Analyzing documents...' : 'Analyze Crime Papers'}
              </button>
            </div>
          </div>
        ...
        <div className="grid" style={{ marginTop: 20 }}>
          <section className="panel section">
            <div className="section__header">
              <h2 className="section__title">Search Crime Papers</h2>
              <span className="section__caption">Analysis type: CRIME</span>
            </div>
            <div className="form form--crime">
              <input
                className="field"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="Crime reporting systems, criminal justice papers..."
          ...
          <section className="cards">
            ...
            <StatCard label="Features Found" value={totalRanked} subtext="Ranked crime features" />
            <StatCard label="Top Share" value={`${topPercentage.toFixed(2)}%`} subtext="Most frequent feature" />
          </section>

          <SummaryPanel summary={analysisResponse.summary} />

          <div className="grid grid--two">
            <RankingBarChart title="Crime Feature Ranking" data={chartData} />
            <RankingPieChart
              title="Crime Feature Distribution"
