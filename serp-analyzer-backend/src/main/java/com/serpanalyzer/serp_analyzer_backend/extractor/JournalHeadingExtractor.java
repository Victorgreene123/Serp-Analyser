...
@Component
public class JournalHeadingExtractor implements FeatureExtractor {
    ...
    public JournalHeadingExtractor(DocumentParser documentParser, PatternMatcher patternMatcher) {
    ...
    public boolean supports(AnalysisType analysisType) {
        return AnalysisType.JOURNAL == analysisType;
    }
    ...
    public ExtractionResult extractFeatures(PageContent content) {
        ...
        return ExtractionResult.headings(patternMatcher.findMatches(normalizedText, keywordDictionary.journalHeadings()));
    }
}
